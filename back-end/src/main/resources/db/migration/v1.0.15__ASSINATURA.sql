create table assinatura (id  bigserial not null, created timestamp not null, updated timestamp, primary key (id), tenant varchar(150), mes_validade varchar(20), ano_validade varchar(50), data_vencimento timestamp, numero_cartao bigint, nome_titular_cartao varchar(150), documento_titular_cartao varchar(20), dataNascimento_titular_cartao timestamp, codigo_area smallint not null, telefone bigint not null, endereco_id bigint);
create table assinatura_aud (id int8 not null, rev int8 not null, revtype int2,  primary key (id, rev), tenant varchar(150), mes_validade varchar(20), ano_validade varchar(50), data_vencimento timestamp, numero_cartao bigint, nome_titular_cartao varchar(150), documento_titular_cartao varchar(20), dataNascimento_titular_cartao timestamp, codigo_area smallint, telefone bigint, endereco_id bigint);

alter table assinatura drop constraint if exists UK_numero_cartao;
alter table assinatura add constraint UK_numero_cartao unique (numero_cartao);
