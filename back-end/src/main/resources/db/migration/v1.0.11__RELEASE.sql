...
create table dispositivo (id  bigserial not null, created timestamp not null, updated timestamp, nome varchar(150) not null, numero_licenca bigint NOT NULL DEFAULT nextval('dispositivo_numero_licenca_seq'::regclass), primary key (id));
create table dispositivo_aud (id int8 not null, rev int8 not null, revtype int2, nome varchar(150), numero_licenca bigint, primary key (id, rev));
